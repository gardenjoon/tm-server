    ...
    */
    try {
        const api_url = encodeURI(
            `https://dapi.kakao.com/v2/local/geo/coord2regioncode.json?x=${req.params.x}&y=${req.params.y}`
        );
...
const search = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await Promise.all(
            result["documents"].map(async (store) => {
                ...
                if (!(await new Controller.ResMenu(Entity.ResMenu, AppDataSource.manager).checkMenu(store.id))) {
                    ...
                    store["menus"] = await new Controller.ResMenu(Entity.ResMenu, AppDataSource.manager).getMenu(
                        store.id
                    );
                }
            })
